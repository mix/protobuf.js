...
tape.test("converters", function(test) {
    ...
    protobuf.load("tests/data/convert_wrappers.proto", function(err, root) {
        ...
        test.test(test.name + " - Message.fromObject - wrapper objects", function(test) {
            var obj = {
                id: 1,
                simpleField: "string value",
                wrappedField: { value: "string value" },
                simpleId: 1,
                wrappedId: { value: 1 },
                simpleBool: true,
                wrappedBool: { value: true },
            };
            var msg = Message.fromObject(obj);
            ...
            test.same(response, expected, "expected and resultant json representation should be the same")
